...
//////////////////////////////////////////////////////////////////////////////////////
MD5::MD5(unsigned char* input, unsigned int length)
{
   init();
   update(input, length);
   finalize();
...
void MD5::update(uint1* input, uint4 input_length)
{
   ...
   if( m_finalized )
   {  // so we can't update!
      _D_.Error("MD5::update:  Can't update a finalized digest!\n");
      return;
   }
...
void MD5::finalize()
{
   ...
   if( m_finalized )
   {
      _D_.Error("MD5::finalize:  Already finalized this digest!\n");
      return;
   }
...
unsigned char*MD5::raw_digest()
{
   if( !m_finalized )
   {
      _DTraceError("MD5::raw_digest:  Can't get digest if you haven't finalized the digest!\n");
      return( (unsigned char*) "");
   }

   uint1* s = (uint1*)malloc(16 * sizeof(uint1));
   if( s == NULL )
   {
      _DTraceError("MD5::raw_digest: Memory error");
      return( (unsigned char*) "");
   }
...
char* MD5::hex_digest()
{
   if( !m_finalized )
   {
      _D_.Error("MD5::hex_digest:  Can't get digest if you haven't finalized the digest!\n");
      return "";
   }
   ...
   if( s == NULL )
   {
      _DTraceError("MD5::hex_digest: Memory error");
      return "";
   }
...
void MD5::transform(uint1* buffer)
{
   uint4 a = m_state[0];
   uint4 b = m_state[1];
   uint4 c = m_state[2];
...
uint4 MD5::rotate_left(uint4 x, uint4 n)
{
   return (x << n) | (x >> (32-n));
}
...
uint4 MD5::F(uint4 x, uint4 y, uint4 z)
{
   return (x & y) | (~x & z);
}

//////////////////////////////////////////////////////////////////////////////////////
uint4 MD5::G(uint4 x, uint4 y, uint4 z)
{
   return (x & z) | (y & ~z);
}

//////////////////////////////////////////////////////////////////////////////////////
uint4 MD5::H(uint4 x, uint4 y, uint4 z)
{
   return x ^ y ^ z;
}

//////////////////////////////////////////////////////////////////////////////////////
uint4 MD5::I(uint4 x, uint4 y, uint4 z)
{
   return y ^ (x | ~z);
}
