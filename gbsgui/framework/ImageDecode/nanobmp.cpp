 ...
 * nonzero on success.
 */
static int ReadLittleBytes(uint32_t * dest, int bytes, void * fp)
{
    uint32_t shift = 0;
    ...
    *dest = 0;
    ...
    {
      uint8_t byte[1];
      if(SYS_FRead(byte, 1, sizeof(byte), fp) != sizeof(byte))
      {
         return 0;
      }
      *dest += byte[0] << shift;
 ...
 * host's byte order.  Returns 0 on EOF or nonzero on success.
 */
static int ReadLittleInt32(int32_t * dest, void * fp)
{
 ...
 */
static int ReadLittleUint16(uint16_t * dest, void * fp)
{
    uint32_t t;
 ...
 * nonzero on success.
 */
static int ReadUint8(uint8_t * dest, void * fp)
{
 ...
 * success.
 */
static int ReadHeader(bmp_header * header, void * fp)
{
 ...
 * success.
 */
static int ReadInfo(bmp_info * info, void * fp)
{
 ...
 */
static int ReadPalette(bmp_palette_entry * palette, size_t colors, void * fp)
{
    ...
    for(i = 0; i < colors; i++)
    {
        ...
        if(SYS_FRead(components, 1, sizeof(components), fp) != sizeof(components))
            return 0;
...
static int Validate(read_context * p_ctx)
{
    do
    {
        if(!ReadHeader(&p_ctx->header, p_ctx->fp))
        {
            break;
        }
        if(!ReadInfo(  &p_ctx->info,   p_ctx->fp)) break;

//        printf("p_ctx->header.magic[0] = %#x, p_ctx->header.magic[1] = %#x \n",p_ctx->header.magic[0],p_ctx->header.magic[1]);
//        printf("Bitmap width, height = (%d,%d) \n",p_ctx->info.width,p_ctx->info.height);
        ...
        if(!(p_ctx->flags & BMPREAD_ANY_SIZE))
        {
             ...
             */
            if(!IsPowerOf2(p_ctx->info.width))
            {
               break;
            }
            if(!IsPowerOf2(p_ctx->lines))
            {
               break;
            }
        }
...
static int Decode(read_context * p_ctx)
{
    ...
    if(SYS_FSeek(p_ctx->fp, p_ctx->header.data_offset, SEEK_SET)) return 0;

    while(p_rgb != p_rgb_end &&
          SYS_FRead(p_ctx->file_data, 1, p_ctx->file_line_len, p_ctx->fp) ==
          p_ctx->file_line_len)
    {
...
int bmpread(const char * bmp_file, unsigned int flags, bmpread_t * p_bmp_out)
{
    ...
    do
    {
        ...
        if(!(ctx.fp = SYS_FOpen(bmp_file, "rb"))) break;
