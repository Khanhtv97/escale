...
void ConnManager::MainTask(void* Parameter)
{
   ...
   while( ConnManager::m_isConnectionMainTaskRuning )
   {
      ...
      {
         _DTraceDebug("Device Is Not Initialized");
         if( ConnManager::getInstance().isDeviceReady() )
         {
            _DTraceDebug("Device Is Ready");
            mainTaskCounter = 0;
            //init again
         ...
         {
            if( mainTaskCounter > 5 )
            {
               //can not wait after 120 seconds
               _DTraceDebug("Can not wait after 120 seconds");
...
{
   m_pConnDevice = NULL;
   m_isSessionInitiated = false;
   m_sessionTimeoutCount = 0;
...
{
   if( ConnManager::m_isConnectionMainTaskRuning == false )
   {
      createTask();
   }
}
...
{
   //Do nothing at the moment, the task should not be stop
   //deleteTask();
}
...
void ConnManager::submitItemData()
{
   ...
   if( m_itemDataQueues.empty() && m_isSessionInitiated )
   {
      if( m_sessionTimeoutCount > MAX_SESSION_TIMEOUT )
      {
         m_pConnDevice->termSession();
         m_isSessionInitiated = false;
         m_sessionTimeoutCount = 0;
      }
      else
   ...
   if( !m_itemDataQueues.empty() )
   {
      ...
      if( !m_itemDataQueues.empty() && !m_isSessionInitiated )
      {
         m_pConnDevice->initSession();
         m_isSessionInitiated = true;
         m_sessionTimeoutCount = 0;
      }

      int trySubmitCount = 0;
      while( trySubmitCount < MAX_NUM_TRYING_TO_SEND )
      {
         _DTraceDebug("Try to submit ItemData... %d", trySubmitCount+1);
      ...
      // if try to submit false, push the error to end of queue
      g_SemaphoreLockItemDataNotified = false;
      itemData_i = m_itemDataQueues.front();
...
void ConnManager::submitItemDataDeleted()
{
   ...
   if( m_itemDataDeletedQueues.empty() && m_isSessionInitiated )
   {
      if( m_sessionTimeoutCount > MAX_SESSION_TIMEOUT )
      {
         m_pConnDevice->termSession();
   ...
   if( !m_itemDataDeletedQueues.empty() )
   {
...
void ConnManager::submitWeighData()
{
   ...
   if( m_weighDataQueues.empty() && m_isSessionInitiated )
   {
      if( m_sessionTimeoutCount > MAX_SESSION_TIMEOUT )
      {
         m_pConnDevice->termSession();
   ...
   if( !m_weighDataQueues.empty() )
   {
      ...
      while( trySubmitCount < MAX_NUM_TRYING_TO_SEND )
      {
         _DTraceDebug("Try to submit WeighData... %d", trySubmitCount+1);
